...
int main(int argc, char **argv)
{
	if (argc != 3)
	{		//                        1            	2        
		cout<<"Usage: "<<argv[0]<<"  <normal.db>  <trace>"<<endl;
	...
	while (fin2.get(ch))
	{
		...
		else // process the line
		{
			...
			if (syscall_past != -1)
			{
				if (normalDB[syscall_past][syscall_curr] == 0) flags++;
			} 
	...
	delete[] normalDB;
	cout<<"# flags, # patterns, ratio"<<endl;
	cout<<flags<<","<<total<<","<<(double)flags/(double)total<<endl;
}
